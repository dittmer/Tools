# Basic ROOT file diff utility
# Mostly adapted from https://github.com/wiso/ROOTDiff/blob/master/ROOTDiff.py
...
def diff_TH2(histo1, histo2):
    ...
    for ibin in xrange(1, (histo1.GetNbinsX() + 1)):
        if histo1.GetXaxis().GetBinLowEdge(ibin) != histo2.GetXaxis().GetBinLowEdge(ibin):
            lines.append("< different binning (bin {} = {})".format(ibin, histo1.GetXaxis().GetBinLowEdge(ibin)))
            lines.append("> different binning (bin {} = {})".format(ibin, histo2.GetXaxis().GetBinLowEdge(ibin)))
    ...
    for ibin in xrange(1, (histo1.GetNbinsY() + 1)):
        if histo1.GetYaxis().GetBinLowEdge(ibin) != histo2.GetYaxis().GetBinLowEdge(ibin):
            lines.append("< different binning (bin {} = {})".format(ibin, histo1.GetYaxis().GetBinLowEdge(ibin)))
            lines.append("> different binning (bin {} = {})".format(ibin, histo2.GetYaxis().GetBinLowEdge(ibin)))

    bins_different = []
    for ibinx in xrange(1, (histo1.GetNbinsX() + 1)):
        for ibiny in xrange(1, (histo1.GetNbinsY() + 1)):
            ibin = histo1.GetBin(ibinx,ibiny)
            if histo1.GetBinContent(ibin) != histo2.GetBinContent(ibin):
                bins_different.append(ibin)
    ...
    if bins_different:
        ...
        if len(bins_different) > 10:
            to_write = bins_different[:5]
        for ibin in to_write:
            histo1.GetBinXYZ(ibin,ibinx,ibiny,ibinz)
            bin_lo_edge_x = histo1.GetXaxis().GetBinLowEdge(ibinx)
            bin_hi_edge_x = histo1.GetXaxis().GetBinLowEdge(ibinx + 1)
            bin_lo_edge_y = histo1.GetYaxis().GetBinLowEdge(ibiny)
            bin_hi_edge_y = histo1.GetYaxis().GetBinLowEdge(ibiny + 1)
            lines.append("< different content (bin {} ({}, {})x({}, {}) = {})".format(ibin,
    ...
    if bins_different:
        ...
        for ibin in to_write:
            ...
            bin_lo_edge_y = histo1.GetYaxis().GetBinLowEdge(ibiny)
            bin_hi_edge_y = histo1.GetYaxis().GetBinLowEdge(ibiny + 1)
            lines.append("< different error (bin {} ({}, {})x({}, {}) = {})".format(ibin,
                                                                                    bin_lo_edge_x, bin_hi_edge_x,
                                                                                    bin_lo_edge_y, bin_hi_edge_y,
                                                                                    histo1.GetBinError(ibin)))
            lines.append("> different error (bin {} ({}, {})x({}, {}) = {})".format(ibin,
